...
- 12 teaching weeks, each with a 1 h lecture and a 2 h lab; the Synthetic Gallery is in the exam period.
- Lab order is always **Explore (about 40 min), Reflect (about 20 min), Create (about 60 min)**, then the log at home. The acronym is ERC everywhere.
...
- `## This week's lab: Explore, Reflect, Create` — `### Explore (about 40 min)`: pick one text tool and one image tool from the tools page; same brief, two media; vary one thing; save both. `### Reflect (about 20 min)`: in pairs, compare your one-sentence definition from the start of the chapter with the working definition, and say one thing you now intend to make. `### Create (about 60 min)`: assemble one flyer or diptych; write the first log entry using `templates/practice-log.md`; the last 15 minutes present each other's artefacts to a third student. Keep the "Save your prompts" tip.
...
- Lab: Explore (about 40 min): inspect one model card on Hugging Face (dataset, parameters, licence, limitations) and run one prompt through three sampler settings in an image tool; Reflect (about 20 min): pairs discuss distribution versus boundary and what the three samples show, each states one intention; Create (about 60 min): the captioned triptych with a 150-word artist's statement; the training-loop and sampler apps as optional Explore.
...
- Lab: Explore (about 40 min): audit one tool with the four questions (provenance, bias probe of ten prompts, labour, sustainability, labelling), 600–1 000 words started in class; Reflect (about 20 min): the structured debate with drawn positions, two teams, then each student states the essay prompt they will take; Create (about 60 min): the one-page AI policy for an imagined organisation, committed as `ai-policy.md`. The ethics essay block from the current chapter, with the five prompts, moved here and marked **due week 7**.
...
- Lab: Explore (about 40 min): the rhythm-bot session, then map a phone or laptop sensor to a generator with the mapper app or a borrowed kit; Reflect (about 20 min): pairs decide which parts of their own final project should stay embodied and why, each states what they will rehearse; Create (about 60 min): a timed project rehearsal with peer feedback using the gallery-page template, and the final log entry with the week-1 definition beside the new one.
